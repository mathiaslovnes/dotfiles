...
local os_name = handle and handle:read("*a"):match("^%s*(.-)%s*$")
handle:close()

if mp.get_property_native('options/vo-mmcss-profile', o) ~= o then
  platform = 'windows'
elseif os_name == "Darwin" or mp.get_property('options/cocoa-force-dedicated-gpu', o) ~= o then
  platform = 'macos'
else
  platform = 'linux'
end
...
local function create_screenshot(s, e)
  ...
  elseif IMAGE_FORMAT == 'jpeg' or IMAGE_FORMAT == 'jpg' then
    table.insert(cmd, '--vf-add=format=yuvj420p')
  elseif IMAGE_FORMAT == 'png' then
    table.insert(cmd, '--vf-add=format=rgb24')
  end
  -- table.insert(cmd, '--vf-add=scale=480*iw*sar/ih:480')
  table.insert(cmd, '--vf-add=scale=iw:ih') -- Keep original size
...
mp.add_key_binding("Meta+v", "update-anki-card", ex)
mp.add_key_binding("Meta+t", "toggle-clipboard-insertion", toggle_sub_to_clipboard)
mp.add_key_binding("Meta+d", "toggle-debug-mode", toggle_debug_mode)
